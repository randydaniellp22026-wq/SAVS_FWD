...
const VehiclePDFButton = ({ vehicle }) => {
  const generatePDF = async () => {
    try {
      ...
      doc.setFont('helvetica', 'bold');
      doc.text(
        `${String(vehicle.marca || '')} ${String(vehicle.modelo || vehicle.name || '')}`,
        15,
        55
      );
      ...
      const summary = String(
        vehicle.summary || vehicle.heroSubtitle || 'Sin descripción detallada disponible.'
      );
      ...
      doc.text(
        '© 2024 IMPORTADORA SAVS - San José, Costa Rica. Documento generado para fines informativos.',
        pageWidth / 2,
        290,
        { align: 'center' }
      );
