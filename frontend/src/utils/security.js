...
export const sendEmail = async (userEmail, userName, code) => {
  ...
  try {
    ...
    if (SECURITY_CONFIG.emailjs.publicKey === 'YOUR_PUBLIC_KEY') {
      console.warn(
        'ADVERTENCIA: No se han configurado las llaves de EmailJS en src/utils/security.js. Simulando envío...'
      );
      return new Promise((resolve) => setTimeout(resolve, 1500));
    }
    ...
    const templateParams = {
      ...
      current_date: now.toLocaleDateString('es-CR', {
        day: '2-digit',
        month: 'long',
        year: 'numeric',
      }),
      current_time: now.toLocaleTimeString('es-CR', {
        hour: '2-digit',
        minute: '2-digit',
        second: '2-digit',
      }),
