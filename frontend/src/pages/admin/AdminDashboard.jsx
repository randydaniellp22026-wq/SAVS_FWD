    ...
    <div className="admin-dashboard-overview">
      <div className="admin-header">
        <h1 className="admin-title">
          Panel de Control <ShieldCheck size={36} className="admin-icon-title" />
        </h1>
        <p className="admin-subtitle">
          Bienvenido, {user.nombre}. Gestiona el inventario y usuarios de SAVS.
        </p>
      </div>
            ...
            <div className="chart-wrapper">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip
                    contentStyle={{ background: '#000', border: '1px solid #333', color: '#fff' }}
                  />
                </PieChart>
              </ResponsiveContainer>
            </div>
          ...
          <div className="chart-card tradein-full-chart">
            <div className="tradein-chart-header">
              <div>
                ...
                <p className="tradein-chart-desc">
                  Análisis de distribución de vehículos de clientes en proceso de canje.
                </p>
              </div>
              ...
              <div key={item.id} className="tradein-item">
                <div className="tradein-info-content">
                  ...
                  <div className="tradein-text">
                    <h4 className="tradein-model">
                      {item.marca} {item.modelo} ({item.anio})
                    </h4>
                    <p className="tradein-price">
                      Precio pretendido:{' '}
                      <span className="price-value">₡{parseInt(item.precio).toLocaleString()}</span>
                    </p>
                    <div className="tradein-status-container">
                      <span
                        className={`tradein-badge status-${item.estado?.toLowerCase().replace(' ', '-') || 'revision'}`}
                      >
                        {item.estado || 'En revisión'}
                      </span>
                      {item.respuesta_admin && (
                        <span className="admin-note">- {item.respuesta_admin}</span>
                      )}
                    </div>
                  </div>
                </div>
      ...
      <div className="server-status-footer">
        ...
        <h3 className="server-status-title">
          {stats.serverStatus?.title || 'ESTADO DEL SERVIDOR'}
        </h3>
        <div className="server-status-indicator">
          <div
            className={`status-dot ${stats.serverStatus?.is_online ? 'online' : 'offline'}`}
          ></div>
          <span className={`status-text ${stats.serverStatus?.is_online ? 'online' : 'offline'}`}>
            {stats.serverStatus?.status_text || 'SISTEMA EN LÍNEA'}
          </span>
        </div>
        <p className="server-region-text">
          {stats.serverStatus?.region_text ||
