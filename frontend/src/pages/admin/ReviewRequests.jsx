...
const ReviewRequests = () => {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      Swal.fire({
        ...
        icon: 'error',
        title: 'Oops...',
        text: 'No se pudieron cargar las solicitudes desde MySQL.',
      });
    } finally {
      setLoading(false);
    }
  ...
  const updateRequestStatus = async (id, newStatus, replyMsg = null) => {
    try {
      ...
      Swal.fire({
        ...
        icon: 'error',
        title: 'Error',
        text: 'No se pudo actualizar en la base de datos.',
      });
    }
  };
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'accepted':
        return (
          <span className="admin-status-badge accepted">
      ...
      case 'replied':
        return (
          <span className="admin-status-badge replied">
            <Send size={14} /> Respondida
          </span>
        );
      default:
        return null;
    ...
    <div className="admin-container">
      <div className="admin-header">
        ...
        <p className="admin-subtitle">
          Gestiona en tiempo real las consultas y solicitudes enviadas desde la página de contacto.
        </p>
      </div>
            ...
            <div key={req.id} className="request-card">
              <div className="request-card-header">
                <div>
                  ...
                  <span className="req-subject">
                    <FileText size={14} /> {req.subject}
                  </span>
                </div>
              ...
              <div className="request-card-meta">
                <span>
                  <Mail size={16} /> {req.user_email}
                </span>
                <span>
                  <Phone size={16} /> {req.user_phone}
                </span>
                <span>
                  <Calendar size={16} /> {formatDate(req.date)}
                </span>
              </div>
              ...
              <div className="request-card-actions">
                ...
                {req.status !== 'rejected' && (
                  <button className="btn-action reply" onClick={() => handleAction(req, 'reply')}>
